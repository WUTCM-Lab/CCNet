...
class CCNet(nn.Module):
    ...
    def __init__(self, in_channels=1,):
        ...
        self.SDE = SpatialDiffEnhancement(d_channels=CHANNEL, e_channels=CHANNEL*2)
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=8 * (CHANNEL * 2 - 2), out_channels=100, kernel_size=(3, 3)),
            nn.BatchNorm2d(100),
            nn.ReLU(),
        )

        self.cla = Classifier()
        self.SA_LSA = SpectralAttention_SpectralLinearAttention(dim=100, input_resolution=[10, 10], num_heads=10)

    def forward(self, x1, x2):

        D = x1 - x2
        E = torch.cat((x1, x2), dim=1)  # (B, 448, H, W)

        E_enhanced = self.SDE(D, E)  # (B, 448, H, W)

        x = self.conv3d_features(E_enhanced.unsqueeze(1))
...
if __name__ == '__main__':
    ...
    model.eval()
    print(model)
    input1 = torch.randn(1, 224, 14, 14)
    input2 = torch.randn(1, 224, 14, 14)
